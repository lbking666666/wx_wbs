module.exports = class Captcha {
  ...
  init() {
    ...
    this.ctx.setFillStyle('rgba(255,255,255)');
    this.ctx.fillRect(0, 0, this.options.width, this.options.height);
    this.ctx.drawImage('http://xm-file.oss-cn-beijing.aliyuncs.com/wchat/capthca-line.jpg', this.randomNum(-300, 0), 0, 400, 32);
  }
  refresh(code) {
    let arr = (code + '').split('');
    if (arr.length === 0) {
    ...
    arr.forEach((item, index) => {
      ...
      let dis = offsetLeft * index + marginLeft - size * 0.5;
